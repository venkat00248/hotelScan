...
export const View = ({ ticket, ticketID, ticketLoading = true }: Props) => {
 ...
 useEffect(() => {
  ...
  const fetchData = async (rfcno: string) => {
    try {
      const response = await TicketTypeService.getSerachResultsByTicket(rfcno);
      if (response) {
        setloading(false);
  ...
  const workflowDetail = async (id:any) => {
    setloading(true);
    try {
      const response = await TicketTypeService.getWorkFlowById(id)
  ...
  const handleNext = async () => {
    try{
        const res = await TicketTypeService.updateTicket({
  ...
  const fetchWorkflowData  = async ()=>{
    ...
    try{
      const response = await TicketTypeService.getMappedWorkflow()
