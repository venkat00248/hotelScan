...
export default function CloseTickets() {
  ...
  const fetchData = async () => {
    try {
      const res = await TicketTypeService.getDeptOpenTicket();
  ...
  const fetchCloseTicket = async (id = 20648) => {
    try {
      const res = await TicketTypeService.getcloseTicketByOwnerId(sessionStorage.staffId);
      const lists = res?.data?.data?.list || [];
      setClosedTickets(res?.data?.data?.list);
