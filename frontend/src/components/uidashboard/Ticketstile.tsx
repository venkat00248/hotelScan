...
export const Ticketstile = ({
  ...
  const fetchSLA = async (list: any = []) => {
    ...
    if (list.length === 0) {
      ...
      const res = await TicketTypeService.getSLADetailsByStaffId(id);
  ...
  const fetchUnassign = async (limit: number = lconfig.API.LIMIT, offset: number = 1) => {
    try {
      const res = await TicketTypeService.getUnAssignedTickets(limit, offset);
  ...
  const fetchTicketsByDate = async (
    ...
    offset: number = 0
      ...
      const res = await TicketTypeService.getOpenTicketsByDateRange(
        limit,
        offset,
